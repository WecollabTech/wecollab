                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        >
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    >
                                        <Link
                                            ...
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition-all duration-200 text-sm font-medium"
                                        ...
                                        <!-- Botón para crear subcategorías -->
                                        <Link
                                            :href="`/subcategorias/create?categoria_id=${categoria.id}`"
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg transition-all duration-200 text-sm font-medium"
                                        ...
                                        <button
                                            ...
                                            "
                                            class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-lg transition-all duration-200 text-sm font-medium"
                            ...
                            <tr v-else-if="categorias.data?.length === 0">
            ...
            >
                <div class="flex items-center gap-3">
                    <span class="text-sm text-gray-600 dark:text-gray-400">
                        Mostrando {{ categorias.from || 0 }} -
                        {{ categorias.to || 0 }} de {{ categorias.total || 0 }}
                    </span>
                ...
                <div class="flex gap-1 flex-wrap justify-center">
                    ...
                    <button
                        @click="obtenerCategorias(categorias.current_page - 1)"
                        :disabled="categorias.current_page === 1"
                        class="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
                    >
                        Anterior
                    ...
                    <button
                        @click="obtenerCategorias(categorias.last_page)"
                        :disabled="
                            categorias.current_page === categorias.last_page
                        "
                        class="px-3 py-1.5 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
...
const categoriasActivas = computed(() => {
    return (
        categorias.value.data?.filter((c) => c.estado === "activo").length || 0
    );
});

const categoriasInactivas = computed(() => {
    return (
        categorias.value.data?.filter((c) => c.estado === "inactivo").length ||
        0
    );
});
...
const paginasMostradas = computed(() => {
    const current = categorias.value.current_page;
    const last = categorias.value.last_page;
    const delta = 2;
    const range = [];
...
const obtenerCategorias = async (page = 1) => {
    ...
    try {
        ...
        const response = await axios.get(`/categorias?${params}`);
...
const recargarDatos = () => {
    obtenerCategorias(categorias.value.current_page);
...
const eliminar = async () => {
    ...
    try {
        await axios.delete(`/categorias/${categoriaAEliminar.value.id}`);
        modalVisible.value = false;
        mostrarToast("Categoría eliminada correctamente", "success");

        const paginaActual =
            categorias.value.data.length === 1 &&
            categorias.value.current_page > 1
                ? categorias.value.current_page - 1
                : categorias.value.current_page;

        await obtenerCategorias(paginaActual);
    } catch (error) {
...
const exportarDatos = async (soloFiltrados = true) => {
    ...
    try {
        ...
        const url = `/categorias/export${params.toString() ? "?" + params.toString() : ""}`;
        ...
        let filename = `categorias_${new Date().toISOString().slice(0, 19)}.csv`;
...
const copiarAlPortapapeles = async () => {
    ...
    try {
        ...
        const response = await fetch(`/categorias/export?${params.toString()}`);
